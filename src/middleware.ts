...
const PUBLIC_PREFIXES = [
   ...
   * pdf.js 의 CMap·기본 글꼴.
   *
   * 뷰어가 이걸 **주소로 받아 간다.** 로그인으로 리다이렉트되면 pdf.js 는
   * 로그인 페이지 HTML 을 CMap 으로 알고 읽으려다 조용히 실패하고, 한글·한자
   * 논문의 글자가 통째로 빠진다. 쿠키가 붙는 요청이라 지금도 대개 통과하지만,
   * 세션이 막 끊긴 순간에 그런 모양이 된다.
   *
   * 여기에 비밀은 없다 — pdfjs-dist 패키지에 그대로 들어 있는 공개 자산이다.
   */
  "/pdfjs/",
];
